...
async function main() {
  ...
  const actless = await prisma.$queryRawUnsafe<{ id: number; fullName: string }[]>(`
    SELECT a.id, a.fullName FROM Act a
    LEFT JOIN GazetteEntry e ON e.actId = a.id AND e.isPrimary = 1
    WHERE a.actType IN ('พระราชบัญญัติ','พระราชกำหนด','พระราชบัญญัติประกอบรัฐธรรมนูญ')
