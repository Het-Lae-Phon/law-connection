...
export function SearchBox({
  ...
  return (
    <div ref={boxRef} className="relative flex-1">
      <form action="/search" className="flex gap-2">
        <div className="relative flex-1">
          <input
            ...
            autoFocus={autoFocus}
            autoComplete="off"
            className="w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-amber-500"
          />
          {loading && (
      ...
      {open && (
        <div className="absolute z-20 mt-1 w-full rounded-lg border border-slate-200 bg-white shadow-lg text-left overflow-hidden">
          {acts.length > 0 && (
            <div>
              <div className="px-3 pt-2 pb-1 text-xs font-semibold text-slate-400">
                กฎหมายแม่บท
              </div>
              {acts.map((a) => (
                <button
            ...
            <div className="border-t border-slate-100">
              ...
              {entries.map((e) => (
                ...
                >
                  ...
                  {e.actName && (
                    <div className="text-xs text-amber-700">↳ {e.actName}</div>
                  )}
                </button>
              ))}
            </div>
          ...
          <button
            ...
            className="block w-full text-left px-3 py-2 border-t border-slate-100 text-sm text-slate-600 hover:bg-slate-50"
          >
