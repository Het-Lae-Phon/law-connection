...
function Payload({ payload }: { payload: string | null }) {
  ...
  return (
    <dl className="text-sm text-stone-600 space-y-0.5">
      {Object.entries(data)
        .filter(([, v]) => v)
        .map(([k, v]) => (
          <div key={k}>
    ...
    <div className="space-y-8">
      ...
      <header className="space-y-2">
        ...
        <div className="flex flex-wrap gap-4 text-sm text-stone-500">
          {stats.map((s) => (
            <span key={s.type}>
              <b className="text-stone-900">{s._count.toLocaleString("th-TH")}</b>{" "}
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-bold">
          คิวรอตรวจสอบ{" "}
          <span className="text-sm font-normal text-stone-400">{pending.length} รายการ</span>
        </h2>
        {pending.length === 0 && <p className="text-stone-500 text-sm">ไม่มีรายการค้าง</p>}
        {pending.map((c) => (
          <div key={c.id} className="rounded-lg border border-seal-300 bg-white p-4 space-y-2">
            <div className="flex flex-wrap items-baseline gap-2">
              <span className="rounded bg-seal-100 text-seal-800 text-xs px-1.5 py-0.5">
            ...
            {c.act && (
              <div className="text-sm">
                <span className="text-stone-500">กฎหมายแม่บท: </span>
                <Link href={`/act/${c.act.id}`} className="text-seal-700 hover:underline">
                  {c.act.fullName}
                </Link>
              </div>
            )}
            ...
            {c.comment && <div className="text-sm text-stone-700">{c.comment}</div>}
            <div className="flex gap-2 pt-1">
              ...
              <form action={moderate}>
                ...
                <button className="rounded border border-stone-300 px-3 py-1.5 text-sm hover:bg-stone-100">
                  ไม่รับ
                </button>
              </form>
            </div>
      ...
      <section className="rounded-lg border border-dashed border-stone-300 bg-white p-5 space-y-3">
        ...
        <form action={suggestAct} className="grid gap-2 sm:grid-cols-2">
          ...
          <input
            ...
            className="rounded border border-stone-300 p-2 text-sm"
          />
          <button className="rounded bg-stone-900 text-white px-4 py-2 text-sm hover:bg-stone-700 sm:col-span-2 justify-self-start">
            ส่งเข้าคิวตรวจสอบ
          </button>
        </form>
      </section>
        ...
        <ul className="divide-y divide-stone-200 rounded-lg border border-stone-200 bg-white text-sm">
          {recent.map((c) => (
            <li key={c.id} className="p-3 flex flex-wrap items-baseline gap-2">
              ...
              {c.entry && <span className="text-stone-500 truncate max-w-md">{c.entry.title}</span>}
              {c.act && !c.entry && <span className="text-stone-500 truncate max-w-md">{c.act.fullName}</span>}
